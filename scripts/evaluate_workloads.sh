...
do
  ...
  do
    result_path=${result_dir}'/'${workload}'-'${req_dist}'-'${flow_para}'.txt'
    echo '' > ${result_path}
  done
  # Evaluate workloads
    ...
    do
      ...
      do
        ...
        do
          ...
          result_path=${result_dir}'/'${workload}'-'${req_dist}'-'${flow_para}'.txt'
          echo 'Batch size ['${batch_size}'] Algorithm ['${algo}'] Workload ['${workload_name}'] Flow Para ['${flow_para}']'
          for i in $(seq 1 $repeat);
          do
            echo `${exec} ${algo} ${batch_size} ${workload_path} ${key_type[$workload]} ${weights_path}` >> ${result_path}
          done
        done
      done
    done
