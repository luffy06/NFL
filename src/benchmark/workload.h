...
void GenerateSyntheticKeys(DType dist, int num_keys, std::vector<KT>& keys, 
                            std::string path = "") {
...
void LoadData(std::string path, std::vector<std::pair<KT, VT>>& kvs) {
  std::mt19937_64 gen(kSEED);
...
template<typename KT, typename VT>
void LoadRequests(std::string path, std::vector<std::pair<KT, VT>>& load_data, 
...
template<typename KT, typename VT>
void WriteRequests(std::string path, 
...
template<typename KT, typename VT>
void AssessData(const std::pair<KT, VT>* kvs, uint32_t size, bool pretty=false) {
  int num_unordered = 0;
  ...
  if (num_unordered == 0) {
    ...
    ConflictsInfo* ci = BuildLinearModel<KT, VT>(kvs, size, model, 2);
