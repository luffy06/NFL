...
template<typename T>
T CutKey(T key, size_t len, bool verbose=false) {
  ...
  return STRTONUM<std::string, T>(key_cut_str);
}

template<typename T, typename P>
void Format(std::string data_dir, std::string data_name, std::string suffix, int num_keys = 0) {
  std::string source_path = PathJoin(data_dir, data_name + suffix);
  for (int i = 0; i < data_name.size(); ++ i) {
    if (data_name[i] == '_') {
      data_name[i] = '-';
    }
  }
  std::string output_path = PathJoin(data_dir, data_name + ".bin");
  ...
  if (sizeof(T) > sizeof(P) || (sizeof(T) == sizeof(P) && !std::numeric_limits<T>::is_signed)) {
    ...
    ASSERT(std::numeric_limits<T>::is_integer, "Cannot cut the non-integer type");
    size_t key_len = std::numeric_limits<P>::digits10;
    for (int i = 0; i < origin_keys.size(); ++ i) {
      origin_keys[i] = CutKey<T>(origin_keys[i], key_len);
    }
  }
  ...
  for (int i = 0; i < num_keys; ++ i) {
    if (i == 0 || !EQ<T>(origin_keys[i], origin_keys[i - 1])) {
...
int main(int argc, char* argv[]) {
  ...
  if (data_type == "uint64") {
    Format<uint64_t, double>(PathJoin(base_dir, "data"), data_name, std::string("_") + data_type);
  } else if (data_type == "float64") {
    int num_keys = GetNumKeys(data_name);
    Format<double, double>(PathJoin(base_dir, "data"), data_name, ".bin.data", num_keys);
  } else if (data_type == "int64") {
    int num_keys = GetNumKeys(data_name);
    Format<long long, double>(PathJoin(base_dir, "data"), data_name, ".bin.data", num_keys);
  } else {
